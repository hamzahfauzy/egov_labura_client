    ...
    <div class="page-content" v-if="loaderStatus==false" ref="absen">
        ...
        <div class="card card-style">
            <div class="content mb-0">        
                <h3>Filter Izin Kerja Bawahan</h3>
                <p>
        ...
        <div class="card card-style" v-if="logs.length">
            <div class="content">
                <h4>Izin Kerja Bawahan</h4>
                <p>
                    Izin Kerja bawahan pada bulan {{periode.bulan}} dan tahun {{periode.tahun}}
                </p>
            </div>
                    ...
                    <div :id="'collapse'+i" class="bg-theme collapse" :data-bs-parent="'#accordion-1'" style="">
                        ...
                        <div class="row mb-0" v-if="log.status == 'Menunggu'">
                            ...
                            <div class="col-4 ps-1">
                                <a href="javascript:void(0)" @click="showLogAbsenImage(log.lampiran)" class="btn btn-3d btn-s btn-full mb-3 rounded-xs text-uppercase font-900 shadow-s  border-blue-dark bg-blue-light">Lihat</a>
                            </div>
                        </div>
                        <div v-else>
                            <a href="javascript:void(0)" @click="showLogAbsenImage(log.lampiran)" class="btn btn-3d btn-s btn-full mb-3 rounded-xs text-uppercase font-900 shadow-s  border-blue-dark bg-blue-light">Lihat</a>
...
export default {
    ...
    methods:{
        ...
        async tolak(log){
            this.$refs.izinpreloader.toggle()
            var request = await this.$store.dispatch('izinkerja/tolakIzin',log)
            if(request.status == 'berhasil')
            {
                this.$refs.izinpreloader.toggle()
                this.success = {
        ...
        async terima(log){
            this.$refs.izinpreloader.toggle()
            var request = await this.$store.dispatch('izinkerja/terimaIzin',log)
            if(request.status == 'berhasil')
            {
                this.$refs.izinpreloader.toggle()
                this.success = {
