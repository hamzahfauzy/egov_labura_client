        ...
        <div class="footer">
            <div class="footer card card-style mx-0 mb-0">   
                ...
                <form method="post" @submit="doLogin" class="content mb-0">
                    ...
                    <div class="mb-4">
                        <button class="btn btn-m mt-2 mb-2 w-100 bg-blue-dark rounded-sm text-uppercase font-900">Reset Password</button>
                        <center>
                            <router-link :to="{name:'Login'}">Kembali Ke Login</router-link>
                        </center>
                    </div>
...
export default {
    data(){
        return {
            warningMessage:'Reset Password Gagal!',
            dialogStatus: false,
    ...
    methods:{
        async doLogin(event){
            ...
            this.$store.dispatch('auth/resetPassword',this.auth).then(response => {
                this.$refs.preloader.toggle()
